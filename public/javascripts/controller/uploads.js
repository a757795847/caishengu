var images = [], imagetoken =[],imagelis =[];
...
function imagetokenll(){
    $.ajax({
        ...
        async:false,
        success:function(data){
            imagelis.push(data.token);
            imagelis.push(data.file_key);
...
function newUpbrowse(imgNumber){
    var uploader = Qiniu.uploader({
        runtimes: 'html5,flash,html4',
        browse_button: 'pushadd',
        container: 'imagebox',
        uptoken: imagelis[0],
        unique_names: false,
        multi_selection: false,
        ...
        init: {
            ...
            'FileUploaded': function (up, file, info) {
                if(imagelis[1] != undefined){
                    images.push(imagelis[1]);
                }
                var domain = up.getOption('domain');
                var res = eval('(' + info + ')');
                var Src = 'http://' + backend_host + '/other/file/' + res.key + '?' + token;
                var imageBoxs = '';
                imageBoxs += '<div class="imgBox"><button type="button" data-name="'+ imagelis[1] +'" class="close" data-dismiss="alert" aria-hidden="true">×</button>';
                imageBoxs += '<img src="'+ Src +'"></div>'
